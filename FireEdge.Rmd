...
anybody reviewing this file an opportunity to reproduce the analyses and come
up with the conclusions on their own machine. This document and the code
contained herein was written with R version 4.0.2.

This file walks the user through a validation study in connection with the 
*FireEdge* test, which is a cognitive and situation-based judgmenet test
...
Such evidence would lend credence to an organization using the test to make
selection-based decisions with the *FireEdge* and will additionally serve as
a form of legal defensibility for the usage of such a test.

The first bit of code, presented below, loads the tidyverse package to help
with the manipulation of the dataset.
...
Once the tidyverse has been loaded, we can proceed to import a cleaned and
scored dataset that contains a number of different variables required for 
conducting this research study. The code below imports the data table into our
local R environment.
...
helps ensure our study stays on track and we do not get side-tracked with variables
that were not intended to be researched further.
...
The "chr" after the word Study indicates it is a character vector. We need to
convert it to a factor instead as that is the proper categorization. Since we
have included three seperate study locations, a factor variable would make more
sense here because all three levels are explicitly known. The following code 
converts the Study variable from a character vector to a factor vector.

```{r recode_study, include=TRUE}
val_data_tidy <-
  val_data_tidy %>% 
  mutate(Study = as_factor(Study))

str(val_data_tidy$Study) #Check my work
```
...
```
We see that the agency in Alabama had a total of 27  missing cases of supervisor
ratings, while the Colorado agency had a total of 13 missing cases. The average
scores are all consistent across the three study locations, as are the standard
deviations, minimum and maximum scores. Next, we perform the same type of analysis
for the *FireEdge* score.
...
We remove the missing data points and determine we have a total of `r nrow(val_data_tidy)`
...
The second pie chart counts the Race/Ethnicity breakdown for participants. We can see
...
on to conducting correlational analyses and linear regression modelling.
...
in value, the other variable decreases. For the purposes of this study, we seek
...
cor.test(x = val_data_tidy$FireEdge_Score,
         y = val_data_tidy$JobPerformance,
         exact = TRUE,
         method = "pearson")
```
...
of the observed variance being explained by the predictor. The ovearll F-test was
...
lin_reg_graph <-
ggplot(val_data_tidy, aes(FireEdge_Score, JobPerformance)) +
...
summary(
aov(FireEdge_Score ~ Race, data = val_data_tidy)
)
```
